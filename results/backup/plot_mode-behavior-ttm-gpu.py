...
def main(argv):
	...
	gpu_mode_gflops_hicoo_m1 = gpu_mode_gflops_hicoo_m2 = gpu_mode_gflops_hicoo_m3 = []
	
	####### TTM #########
	op = 'ttm'
	R = 16
	gpu_mode_gflops_hicoo_m1, gpu_mode_gflops_hicoo_m2, gpu_mode_gflops_hicoo_m3 = get_ttm_data(op, intput_path, plot_tensors, tensors, nnzs, R, ang_pattern, prefix)
	rects1, rects2, rects3 = plot_gragh_left(ax1, plot_tensors, "TTM", np.asarray(gpu_mode_gflops_hicoo_m1), np.asarray(gpu_mode_gflops_hicoo_m2), np.asarray(gpu_mode_gflops_hicoo_m3))

	# fig.legend([], ['oral', 'physa'], bbox_to_anchor=(2, 0),loc = 'lower right')
	# fig.legend(*fig.axes[0,0].get_legend_handles_labels())

	fig.legend([rects1, rects2, rects3], ["m1", "m2", "m3"], loc = 'upper right') # bbox_to_anchor=(0.5, 0)
...
def plot_gragh_left(ax, plot_tensors, title, o1, o2, o3):
	...
	rects1 = ax.bar(left=ind, height=o1, width=mywidth, color='royalblue', zorder=2, lw=0.5, label='m1')
	rects2 = ax.bar(left=ind + mywidth, height=o2, width=mywidth, color='limegreen',  zorder=2, lw=0.5, label='m2')
	rects3 = ax.bar(left=ind + mywidth * 2, height=o3, width=mywidth, color='m',  zorder=2, lw=0.5, label='m3')

	ax.set_title(title, fontsize=20)
	ax.set_ylabel('Performance (GFLOPS)', fontsize=16)
	ax.set_xticks(ind + mywidth * 1.5)
	ax.set_xticklabels(xnames, fontsize=12, rotation=90)

	ax.set_xlim(min(ind), max(ind) + mywidth * 3)
...
def get_ttm_data(op, intput_path, plot_tensors, tensors, nnzs, R, ang_pattern, prefix):
	...
	# Only for 3D tensors now
	assert(nmodes == 3)
	gpu_mode_gflops_hicoo_m1 = []
	gpu_mode_gflops_hicoo_m2 = []
	gpu_mode_gflops_hicoo_m3 = []
	for i in range(len(gpu_mode_gflops_hicoo)):
		if i % nmodes == 0:
			gpu_mode_gflops_hicoo_m1.append(gpu_mode_gflops_hicoo[i])
		elif i % nmodes == 1:
			gpu_mode_gflops_hicoo_m2.append(gpu_mode_gflops_hicoo[i])
		elif i % nmodes == 2:
			gpu_mode_gflops_hicoo_m3.append(gpu_mode_gflops_hicoo[i])

	print("gpu_mode_gflops_hicoo_m1:")
	print(gpu_mode_gflops_hicoo_m1)
	print("gpu_mode_gflops_hicoo_m2:")
	print(gpu_mode_gflops_hicoo_m2)
	print("gpu_mode_gflops_hicoo_m3:")
	print(gpu_mode_gflops_hicoo_m3)

	assert(len(gpu_mode_gflops_hicoo_m1) == len(nnzs))
	assert(len(gpu_mode_gflops_hicoo_m2) == len(nnzs))
	assert(len(gpu_mode_gflops_hicoo_m3) == len(nnzs))

	# return gpu_gflops_coo, gpu_gflops_hicoo
	return gpu_mode_gflops_hicoo_m1, gpu_mode_gflops_hicoo_m2, gpu_mode_gflops_hicoo_m3
