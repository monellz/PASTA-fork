...
/**
 * Benchmark element-wise COO tensor division. 
...
int main(int argc, char *argv[]) {
    ...
    if(dev_id == -2) {
        sptAssert(sptSparseTensorDotDivEq(&Z, &X, &Y, collectZero) == 0);
    } else if(dev_id == -1) {
        ...
        sptAssert(sptCudaSparseTensorDotDivEq(&Z, &X, &Y, collectZero) == 0);
    }
