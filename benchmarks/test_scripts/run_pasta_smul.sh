...
do
	# Sequetial code
	dev_id=-2
	myprogram="./build/benchmarks/${prog_name} -X ${tsr_path}/${tsr_name}.tns -a ${a_value} -d ${dev_id} > ${out_path}/${tsr_name}_${prog_name}-seq.txt"
	echo ${myprogram}
	# ${myprogram}
	...
	myprogram="./build/benchmarks/${prog_name} -X ${tsr_path}/${tsr_name}.tns -a ${a_value} -d ${dev_id} > ${out_path}/${tsr_name}_${prog_name}-t${nt}.txt"
	echo ${myprogram}
	# ${myprogram}

	# CUDA code
	dev_id=${gpu_dev_id}
	myprogram="./build/benchmarks/${prog_name} -X ${tsr_path}/${tsr_name}.tns -a ${a_value} -d ${dev_id} > ${out_path}/${tsr_name}_${prog_name}-gpu.txt"
	echo ${myprogram}
	# ${myprogram}
