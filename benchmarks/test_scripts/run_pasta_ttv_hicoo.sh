...
do
	...
	do
		...
		myprogram="numactl --interleave=all ./build/benchmarks/${prog_name} -i ${tsr_path}/${tsr_name}.bin -m ${mode} -d ${dev_id} -b ${sb} > ${out_path}/${tsr_name}_${prog_name}-m${mode}-b${sb}-t${nt}.txt"
		echo ${myprogram}
		eval ${myprogram}
