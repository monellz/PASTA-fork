...
do
	...
	do
		...
		do
			...
			myprogram="numactl --interleave=all ./build/benchmarks/${prog_name} -i ${tsr_path}/${tsr_name}.bin -m ${mode} -d ${dev_id} -r ${R} > ${out_path}/${tsr_name}_${prog_name}-m${mode}-r${R}-t${nt}.txt"
			echo ${myprogram}
			eval ${myprogram}
