...
int main(int argc, char *argv[]) {
    ...
    if(dev_id == -2) {
        sptAssert(sptSparseTensorDotMulEq(&Z, &X, &Y, collectZero) == 0);
    } else if(dev_id == -1) {
        ...
        printf("\nnthreads: %d\n", nthreads);
        sptAssert(sptOmpSparseTensorDotMulEq(&Z, &X, &Y, collectZero) == 0);
#endif
    } else {
        sptCudaSetDevice(dev_id);
        sptAssert(sptCudaSparseTensorDotMulEq(&Z, &X, &Y, collectZero) == 0);
    }
    ...
    sptStopTimer(timer);
    sptPrintAverageElapsedTime(timer, niters, "Average CooDotMulEq");
    sptFreeTimer(timer);
