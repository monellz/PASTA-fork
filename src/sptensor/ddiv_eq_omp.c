...
int sptOmpSparseTensorDotDivEq(sptSparseTensor *Z, const sptSparseTensor *X, const sptSparseTensor *Y, int collectZero)
{
    ...
    if(Y->nmodes != X->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp SpTns DotDiv", "shape mismatch");
    }
    ...
    if(Y->nnz != X->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp SpTns DotDiv", "nonzero distribution mismatch");
    }
    ...
    sptStopTimer(timer);
    comp_time = sptPrintElapsedTime(timer, "Omp SpTns DotDiv");
