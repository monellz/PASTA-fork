...
/**
 * OpenMP parallelized element-wise addition of two sparse tensors, with exactly the same nonzero
 * distribution.
...
int sptOmpSparseTensorDotAddEq(sptSparseTensor *Z, const sptSparseTensor *X, const sptSparseTensor *Y, int collectZero)
{
    ...
    if(Y->nmodes != X->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp SpTns DotAdd", "shape mismatch");
    }
    ...
    if(Y->nnz != X->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp SpTns DotAdd", "nonzero distribution mismatch");
    }
    ...
    sptStopTimer(timer);
    comp_time = sptPrintElapsedTime(timer, "Omp SpTns DotAdd");
