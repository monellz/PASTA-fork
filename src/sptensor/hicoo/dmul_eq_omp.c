 ...
 */
int sptOmpSparseTensorDotMulEqHiCOO(sptSparseTensorHiCOO *hiZ, const sptSparseTensorHiCOO *hiX, const sptSparseTensorHiCOO *hiY, int collectZero)
{
    sptAssert(collectZero == 0);
    /* Ensure X and Y are in same shape */
    if(hiY->nmodes != hiX->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp HiSpTns DotMul", "shape mismatch");
    }
    ...
    if(hiY->nnz != hiX->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp HiSpTns DotMul", "nonzero distribution mismatch");
    }
    ...
    sptStopTimer(timer);
    comp_time = sptPrintElapsedTime(timer, "Omp HiSpTns DotMul");
