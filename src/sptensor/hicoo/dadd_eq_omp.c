...
int sptOmpSparseTensorDotAddEqHiCOO(sptSparseTensorHiCOO *hiZ, const sptSparseTensorHiCOO *hiX, const sptSparseTensorHiCOO *hiY, int collectZero)
{
    ...
    if(hiY->nmodes != hiX->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp HiSpTns DotAdd", "shape mismatch");
    }
    ...
    if(hiY->nnz != hiX->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Omp HiSpTns DotAdd", "nonzero distribution mismatch");
    }
    ...
    sptStopTimer(timer);
    comp_time = sptPrintElapsedTime(timer, "Omp HiSpTns DotAdd");
