 ...
 */
int sptSparseTensorDotSubEqHiCOO(sptSparseTensorHiCOO *hiZ, const sptSparseTensorHiCOO *hiX, const sptSparseTensorHiCOO *hiY, int collectZero)
{
    sptAssert(collectZero == 0);
    sptNnzIndex i;
    /* Ensure X and Y are in same shape */
    if(hiY->nmodes != hiX->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Cpu HiSpTns DotSub", "shape mismatch");
    }
    ...
    if(hiY->nnz != hiX->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Cpu HiSpTns DotSub", "nonzero distribution mismatch");
    }
    ...
    sptStopTimer(timer);
    comp_time = sptPrintElapsedTime(timer, "Cpu HiSpTns DotSub");
