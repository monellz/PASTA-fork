...
static __global__ void spt_dSubKernel(
    sptValue *Z_val, 
 ...
 */
int sptCudaSparseTensorDotSubEqHiCOO(sptSparseTensorHiCOO *hiZ, const sptSparseTensorHiCOO *hiX, const sptSparseTensorHiCOO *hiY, int collectZero)
{
    sptAssert(collectZero == 0);
    sptNnzIndex i;
    /* Ensure X and Y are in same shape */
    if(hiY->nmodes != hiX->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "SpTns DotSubHiCOO", "shape mismatch");
    }
    ...
    if(hiY->nnz != hiX->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "SpTns DotSubHiCOO", "nonzero distribution mismatch");
    }
    int result;
    ...
    spt_CheckCudaError(result != 0, "CUDA SpTns DotSubHiCOO");
    sptValue *X_val = NULL;
    ...
    printf("[CUDA SpTns DotSubHiCOO] spt_dSubKernel<<<%lu, (%lu)>>>\n", nblocks, nthreadsx);
    spt_dSubKernel<<<nblocks, dimBlock>>>(Z_val, X_val, Y_val, hiX->nnz);
    result = cudaThreadSynchronize();
    spt_CheckCudaError(result != 0, "CUDA SpTns DotSubHiCOO kernel");

    sptStopTimer(timer);
    sptPrintElapsedTime(timer, "Cuda SpTns DotSubHiCOO");
