...
int sptCudaSparseTensorDotSubEqHiCOO(sptSparseTensorHiCOO *hiZ, const sptSparseTensorHiCOO *hiX, const sptSparseTensorHiCOO *hiY, int collectZero)
{
    ...
    if(hiY->nmodes != hiX->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Cuda HiSpTns DotSub", "shape mismatch");
    }
    ...
    if(hiY->nnz != hiX->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "Cuda HiSpTns DotSub", "nonzero distribution mismatch");
    }
    int result;
    ...
    printf("[Cuda HiSpTns DotSub] spt_dSubKernel<<<%lu, (%lu)>>>\n", nblocks, nthreadsx);
