 ...
 */
int sptCudaSparseTensorAddScalarHiCOO(sptSparseTensorHiCOO *hiZ, sptSparseTensorHiCOO *hiX, sptValue a)
{
    sptAssert(a != 0.0);
    int result;
    ...
    sptStopTimer(timer);
    comp_time = sptPrintElapsedTime(timer, "Cuda HiSpTns AddScalar");
