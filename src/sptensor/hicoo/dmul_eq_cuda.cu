...
int sptCudaSparseTensorDotMulEqHiCOO(sptSparseTensorHiCOO *hiZ, const sptSparseTensorHiCOO *hiX, const sptSparseTensorHiCOO *hiY, int collectZero)
{
    ...
    if(hiY->nmodes != hiX->nmodes) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "SpTns DotMulHiCOO", "shape mismatch");
    }
    ...
    if(hiY->nnz != hiX->nnz) {
        spt_CheckError(SPTERR_SHAPE_MISMATCH, "SpTns DotMulHiCOO", "nonzero distribution mismatch");
    }
    int result;
    ...
    spt_CheckCudaError(result != 0, "CUDA SpTns DotMulHiCOO");
    sptValue *X_val = NULL;
    ...
    printf("[CUDA SpTns DotMulHiCOO] spt_dMulKernel<<<%lu, (%lu)>>>\n", nblocks, nthreadsx);
    spt_dMulKernel<<<nblocks, dimBlock>>>(Z_val, X_val, Y_val, hiX->nnz);
    result = cudaThreadSynchronize();
    spt_CheckCudaError(result != 0, "CUDA SpTns DotMulHiCOO kernel");

    sptStopTimer(timer);
    sptPrintElapsedTime(timer, "Cuda SpTns DotMulHiCOO");
